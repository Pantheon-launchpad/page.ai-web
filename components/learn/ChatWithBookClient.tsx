...
import { chatSources, bookPersonas, bookGameChips, type ChatSource } from "@/lib/learn-data";
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {chatSources.map((s) => (
            <button
              key={s.id}
              onClick={() => setSource(s)}
              className="group flex items-center gap-3.5 rounded-2xl border border-ink/10 bg-white/50 p-4 text-left transition-colors hover:border-signal/30 hover:bg-signal-soft"
            >
          ...
          <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-ink/20 bg-white/30 p-4 text-center text-ink-faint transition-colors hover:border-signal/40 hover:text-signal-deep">
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-full border border-ink/10 bg-white/50 px-3.5 py-1.5 text-xs font-medium text-ink-soft transition-colors hover:border-ink/20"
        >
          Switch book
        </button>
      </div>
