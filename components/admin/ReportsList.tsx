  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap gap-2">
        {(["all", "open", "reviewing", "resolved", "dismissed"] as const).map(
          (s) => (
            <button
              key={s}
              onClick={() => setFilter(s)}
              className={`rounded-full border px-3.5 py-1.5 text-sm font-medium capitalize transition-colors ${
                filter === s
                  ...
                  : "border-ink/10 bg-surface-2 text-ink-soft hover:border-ink/20"
              }`}
            >
              {s}
            </button>
          ...
          {filtered.map((r) => (
            ...
            >
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-ink">
                    {reasonLabels[r.reason]} &middot;{" "}
                    {r.targetType.replace("_", " ")}
                  </p>
                  <p className="text-xs text-ink-soft">
                    Reported by {r.reportedBy} &middot;{" "}
              ...
              <div className="flex shrink-0 items-center gap-2">
                <span
                  className={`rounded-full px-2.5 py-1 font-mono text-[10.5px] uppercase tracking-wide ${statusTone[r.status]}`}
                >
                  {r.status}
                </span>
