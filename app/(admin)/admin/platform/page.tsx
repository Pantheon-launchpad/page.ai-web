          ...
          <table className="w-full min-w-[520px] text-left text-sm">
            <thead>
              <tr className="border-b border-ink/10 text-ink-faint">
                <th className="py-2.5 pr-4 font-mono text-[10.5px] font-medium uppercase tracking-wide">
                  Role
                </th>
                <th className="py-2.5 font-mono text-[10.5px] font-medium uppercase tracking-wide">
                  Permissions
                </th>
              </tr>
            </thead>
                  ...
                  <td className="py-2.5">
                    <div className="flex flex-wrap gap-1.5">
                      {r.permissions.map((p) => (
                        <span
                          key={p}
                          className="rounded-full bg-signal-soft px-2.5 py-1 font-mono text-[10.5px] text-signal-deep"
                        >
                          {p}
                        </span>
                      ))}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm text-ink">
                    <span className="font-medium">{l.actor}</span> &middot;{" "}
                    {l.action}
                  </p>
        ...
        <div className="glass-card rounded-3xl p-6">
          ...
          <p className="mt-1 text-xs text-ink-soft">
            API uptime, latency, and storage/CDN health
          </p>
            ...
            {health.map((m) => (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <span
                    className={`h-2 w-2 shrink-0 rounded-full ${m.status === "operational" ? "bg-moss" : "bg-ember"}`}
                  />
