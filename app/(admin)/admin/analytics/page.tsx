...
export default async function AdminAnalyticsPage() {
  const { growthTrend, aiUsageBySubject, stats } =
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="glass-card rounded-3xl p-6">
        ...
        <p className="mt-1 text-xs text-ink-soft">
          Cumulative students, last 8 weeks
        </p>
        <div className="mt-4">
      ...
      <div className="glass-card rounded-3xl p-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3">
          {aiUsageBySubject.map((s) => (
            ...
            >
              <span
                className={`h-2.5 w-2.5 rounded-full ${s.color === "signal" ? "bg-signal" : s.color === "ember" ? "bg-ember" : "bg-moss"}`}
              />
              <span className="truncate">{s.name}</span>
              <span className="ml-auto font-mono text-ink-faint">
                {Math.round((s.value / totalAiUsage) * 100)}%
              </span>
            </div>
          ))}
        </div>
