...
export default async function AchievementsPage() {
  const [achievements, { student }] = await Promise.all([AchievementApi.getAchievements(), DashboardApi.getDashboard()]);
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div className="glass-card rounded-2xl px-4 py-2.5 text-center">
          ...
          <div className="font-mono text-[10px] uppercase tracking-wide text-ink-faint">Unlocked</div>
        </div>
      </PageHeader>
      ...
      <div className="glass-card-deep flex items-center gap-5 rounded-3xl p-6">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-display text-lg font-semibold text-white">Level {student.level}</p>
          <p className="text-sm text-white/60">
            {student.coinsIntoLevel}/{student.coinsForNextLevel} coins to Level {student.level + 1}
          </p>
          <div className="mt-2 h-1.5 w-full max-w-xs overflow-hidden rounded-full bg-white/15">
            <div
              className="h-full rounded-full bg-signal"
              style={{ width: `${Math.round((student.coinsIntoLevel / student.coinsForNextLevel) * 100)}%` }}
            />
          </div>
        </div>
      </div>
          ...
          >
            <div
              className={`flex h-12 w-12 items-center justify-center rounded-2xl ${
                a.earned ? "bg-signal-soft text-signal-deep" : "bg-ink/5 text-ink-faint"
              }`}
            >
              <Icon name={a.icon} className="h-5 w-5" />
            </div>
            <p className="text-sm font-semibold text-ink">{a.title}</p>
            <p className="text-xs leading-snug text-ink-soft">{a.description}</p>
            ...
            {a.earned ? (
              ...
              <div className="mt-1 w-full">
                <div className="h-1 w-full overflow-hidden rounded-full bg-ink/10">
                  <div className="h-full rounded-full bg-ink/30" style={{ width: `${a.progress ?? 0}%` }} />
                </div>
                <span className="mt-1 block font-mono text-[10px] text-ink-faint">{a.progress ?? 0}%</span>
              </div>
            )}
          </div>
        ))}
